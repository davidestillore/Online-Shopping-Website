...
app.put("/wishlist/product/add", function (request, response) {
  Product.findOne({ _id: request.body.productId }, function (err, product) {
    if (err) {
      response.status(500).send({ error: "Could not add item into wishlist" });
    } else {
      WishList.update(
        ...
        function (err, wishList) {
          if (err) {
            response
              .status(500)
              .send({ error: "Could not add item into wishlist" });
          } else {
...
app.listen(3001, function () {
  console.log("Swag Shop API runing on port 3001...");
});
